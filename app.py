...
st.title("ðŸ“Š Financial Market Pattern & Indicator Scanner")

# ==========================================================
# 2. TRADINGVIEW STYLE TOP NAVBAR (Original Timeframe Bar)
# ==========================================================
st.markdown("### ðŸŽ›ï¸ TradingView Toolbar & Timeframe Bar")

# Row for Symbol Input and Quick Actions
col_sym, col_btn = st.columns([3, 1])
with col_sym:
    symbol = st.text_input("ðŸ“ Market Asset Symbol (Ticker):", "XAUUSD=X")
with col_btn:
    st.write("##")
...
selected_tf = st.radio(
    "â±ï¸ Select Timeframe (TradingView Standard):",
    options=tf_options,
    index=6, # Default 1D
    horizontal=True
)
...
with st.expander("âš™ï¸ Chart Settings & Display Controls (Goo'aami Muuqaalka Chart-ka)"):
    c_zoom, c_height, c_theme = st.columns([2, 2, 2])
    with c_zoom:
        visible_candles = st.slider("ðŸ” Zoom Level (Shumacyada la arkayo):", min_value=20, max_value=300, value=60, step=10)
    with c_height:
        chart_height = st.slider("ðŸ“ Chart Height (Dhererka Chart-ka):", min_value=350, max_value=1000, value=550, step=50)
    with c_theme:
        chart_theme = st.selectbox(
            "ðŸŽ¨ Background Theme (Midabka Chart-ka):",
...
if run_scan:
    with st.spinner(f"Soo jiidashada xogta kandeellada ee {selected_tf} ({current_setting['period']})..."):
        ...
        if df.empty or len(df) < 14:
            st.error(f"âš ï¸ Xogta kandeellada ee {symbol} ma buuxdo ama ma jirto time-frame-kan. Fadlan xaqiiji Ticker-ka ama baddal Timeframe-ka.")
        else:
            ...
            # ----------------------------------------------------
            st.markdown(f"### ðŸŽ¯ Final Signal Status (Total Candles: `{total_candles}`)")
            
            signal = result['signal']
            if signal == "STRONG BUY":
                st.success(f"### ðŸŸ¢ SIGNAL: STRONG BUY | Pattern: {result['pattern']}")
            elif signal == "STRONG SELL":
                st.error(f"### ðŸ”´ SIGNAL: STRONG SELL | Pattern: {result['pattern']}")
            else:
                st.warning(f"### ðŸŸ¡ STATUS: {signal} | Pattern: {result['pattern']}")
            ...
            # ----------------------------------------------------
            st.markdown("#### ðŸ“ Absolute Trade Execution Levels")
            e1, e2, e3 = st.columns(3)
            
            if signal in ["STRONG BUY", "STRONG SELL"]:
                e1.metric("ðŸŽ¯ Entry Price", f"{result['entry']}")
                e2.metric("ðŸ›‘ Stop Loss (SL)", f"{result['sl']}")
                e3.metric("ðŸ† Take Profit (TP - 1:2 RRR)", f"{result['tp']}")
            else:
                e1.metric("ðŸŽ¯ Entry Price", "Waiting...")
                e2.metric("ðŸ›‘ Stop Loss (SL)", "N/A")
                e3.metric("ðŸ† Take Profit (TP)", "N/A")
            ...
            # ----------------------------------------------------
            st.markdown(f"### ðŸ“ˆ Interactive Chart ({symbol} - {selected_tf})")
            ...
            # ----------------------------------------------------
            with st.expander(">> ðŸ“Œ Click to Toggle Chart Legend / Indicators Keys (Midabyada & Astamaha Chart-ka)"):
                st.markdown("""
                * ðŸŸ¢ **Green Candles / ðŸ”´ Red Candles**: Candlestick Price Movement
                * ðŸŸ§ **Orange Line**: EMA 50
                * ðŸŸ¦ **Light Blue Line**: EMA 200
                * ðŸ”» **Red Triangles**: Pivot High (H) Structural Resistance
                * ðŸ”º **Green Triangles**: Pivot Low (L) Structural Support
                * ðŸŸ¨ / ðŸ©µ **Dash-Dot Lines**: Pattern Resistance & Support Ranges
                """)
