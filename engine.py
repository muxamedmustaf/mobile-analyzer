...
# ==========================================================
# ENGINE.PY - DYNAMIC SWING SCANNER (v4.7 - STRICT CONDITIONS)
# ==========================================================
...
def calculate_indicators(df):
    df = df.copy()
    # 1. المؤشرات المستندة في الكود: المتوسطات الاسية (EMA50 & EMA200) لتحديد الاتجاه
    df["EMA50"] = df["Close"].ewm(span=50, adjust=False).mean()
    df["EMA200"] = df["Close"].ewm(span=200, adjust=False).mean()

    # 1. مؤشر القوة النسبية (RSI) للزخم
    delta = df["Close"].diff()
    ...
    # 1. حساب المدى الحقيقي المتوسط (ATR) لجعل التأرجح ديناميكياً
...
class PatternValidatorPipeline:
    ...
    def __init__(self, df):
        ...
        self.filters = [
            self.time_filter,
            self.trend_filter,  # 2. التحقق الصارم من الاتجاه المضاد السابق للنموذج الانعكاسي
            self.invalidation_filter,
            self.indicator_confirmation_filter,  # 1. التحقق الصارم من المؤشرات (RSI & EMA)
    ...
    def trend_filter(self, p, data):
        """
        2. شرط صارم جداً: التأكد من وجود اتجاه مضاد سابق قبل النمط الانعكاسي
        - للنموذج الهابط (Head & Shoulders): يجب أن يسبقه اتجاه صاعد (القاع الأول L0 أقل من القيعان السابقة).
        """
        idx_l0 = p[0]["idx"]
        ...
        if len(pre_l0_df) > 10:
            ...
            # شرط الاتجاه المضاد الصاعد السابق للنموذج الانعكاسي الهابط
            if past_min > p[0]["val"]:
                return False, None, None
    ...
    def indicator_confirmation_filter(self, p, data):
        """
        1. تطبيق صارم للمؤشرات الفنية (RSI و EMA) لتأكيد الإشارة
        """
        idx_h3 = p[5]["idx"]
...
def detect_all_inverse_head_shoulders(pivots, df):
    ...
    for i in range(len(pivots) - 5):
        ...
        # 2. شرط صارم: التأكد من وجود اتجاه مضاد هابط سابق قبل النموذج الانعكاسي المقلوب (Inverse H&S)
        idx_h0 = p[0]["idx"]
        ...
        # 1. التأكد الصارم من مؤشرات RSI و EMA عند نقطة اكتمال النموذج
...
if __name__ == "__main__":
    print("ENGINE.PY loaded with Strict Indicators & Reversal Trend Validation (v4.7).")
