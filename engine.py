...
class PatternValidatorPipeline:
    ...
    def breakout_filter(self, p, data):
        ...
        # Ø´Ø±Ø· ØµØ§Ø±Ù…: Ø§Ù„ØªØ£ÙƒØ¯ Ù…Ù† Ø£Ù† Ø³Ø¹Ø± Ø§Ù„Ø¥ØºÙ„Ø§Ù‚ Ø§Ù„Ø­Ø§Ù„ÙŠ ÙŠØªØ¯Ø§ÙˆÙ„ Ø­ØµØ±Ø§Ù‹ Ø¨ÙŠÙ† Ø®Ø· Ø§Ù„Ø¹Ù†Ù‚ ÙˆØ§Ù„Ù‡Ø¯Ù
...
def detect_all_head_shoulders(pivots, df):
    ...
    for i in range(len(pivots) - 5):
        ...
        # Require a clear left-shoulder L-H-L reaction.
        left_reaction_up = (
            h1 - l0
        ) / max(abs(l0), 1e-9)
        ...
        # Require a clear right-shoulder reaction from l2 to h3.
        right_reaction_up = (
            h3 - l2
        ) / max(abs(l2), 1e-9)
        ...
        # Ø´Ø±Ø· ØµØ§Ø±Ù…: Ø§Ù„ØªØ£ÙƒØ¯ Ù…Ù† Ø£Ù† Ø§Ù„Ø³Ø¹Ø± Ø§Ù„Ø­Ø§Ù„ÙŠ Ù„Ù… ÙŠØ®Ø±Ø¬ Ø¹Ù† Ø­Ø¯ÙˆØ¯ Ø§Ù„Ø¯Ø®ÙˆÙ„ ÙˆØ§Ù„Ù‡Ø¯Ù
