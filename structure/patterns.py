...
def detect_chart_patterns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(highs) >= 5 and len(lows) >= 5:
        ...
        # 1. Double Top & Double Bottom (Strict Rule: Farqiga waa inuu ka yar yahay 1% -> 0.01)
        dt_diff = abs(h5 - h4) / h4
        if dt_diff < 0.01 and h3 > h4 and h3 > h5:  
            prob = round(92 - (dt_diff * 100), 1)
        ...
        if abs(l5 - l4) < 0.01 and abs(l4 - l3) < 0.01:
        ...
        elif abs(h5 - h4) < 0.005 and l5 > l4:
            p_name = "Ascending Triangle"
            scored_patterns.append((p_name, 87.2))
            pattern_coords[p_name] = [(h_dates[4], h5), (l_dates[4], l5)]
        elif h5 < h4 and abs(l5 - l4) < 0.005:
